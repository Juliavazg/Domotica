...
const char* mqtt_server = "192.168.12.62"; 
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "casa/actuadores/led10") {
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect("ESP32Client")) {
      ...
      client.subscribe("casa/actuadores/led10");
    } else {
...
void loop() {
  ...
  if (botonEstado == HIGH) { // Se presionamos o botón
    if(!ledAceso) {
      ...
      client.publish("casa/actuadores/led10","LED aceso");
      client.publish("casa/sensores/boton10","Botón premido");
    }
    delay(5000);
  } else{
    if (ledAceso){
      ...
      client.publish("casa/actuadores/led10","LED apagado");
    }
  }
}
